...
export class AgentNamingService {
  ...
  public async registerAgent(
    ...
    try {
      ...
      if (!certValidation.valid) {
        ...
        throw new Error(
          `Certificate validation failed: ${certValidation.details}`
        );
      }
      ...
      if (
        threatReport.recommendedActions.includes(
          SecurityAction.REJECT_REGISTRATION
        )
      ) {
        this.logSecurityEvent({
        ...
        throw new Error(
          `Registration rejected due to security concerns. Threat score: ${threatReport.threatScore}`
        );
      }
      ...
      if (
        threatReport.recommendedActions.includes(
          SecurityAction.MONITOR_ACTIVITY
        ) ||
        threatReport.recommendedActions.includes(
      ...
      const agentCard = formatAgentCard(
        name,
        JSON.stringify({
          certificate,
          metadata,
  ...
  public generateMCPManifest(name: string, manifest: object): string {
    try {
      ...
      throw new Error(
        `Failed to generate MCP manifest: ${this.sanitizeErrorMessage(error)}`
      );
    }
  }
  ...
  private validateAgentName(name: string): ValidationResult {
    ...
    if (!/^[a-zA-Z0-9._-]+$/.test(name)) {
      return {
        valid: false,
        error:
          "Agent name can only contain letters, numbers, dots, hyphens, and underscores",
        code: ANSStatus.BAD_REQUEST,
      };
    }
  ...
  private validateMetadata(metadata: any): ValidationResult {
    ...
    if (typeof metadata !== "object" || Array.isArray(metadata)) {
      return {
        valid: false,
        error: "Agent metadata must be an object",
        code: ANSStatus.BAD_REQUEST,
      };
    }
  ...
  private checkRateLimit(ipAddress: string): ValidationResult {
    ...
    if (
      !record ||
      now.getTime() - record.lastReset.getTime() > 60 * 60 * 1000
    ) {
   ...
   */
  private async getRegistrationHistory(
    name: string
    ...
    try {
      ...
      console.error(
        "Error getting registration history:",
  ...
  private logSecurityEvent(event: SecurityEvent): void {
    try {
      ...
      console.warn(
        `[SECURITY EVENT] ${event.severity} - ${event.type}: ${event.details}`
      );
  ...
  /**
   * Resolve agents by capabilities
   * @param capabilities Array of capabilities to search for
   * @param matchAll Whether to match all capabilities (true) or any capability (false)
   * @returns Promise that resolves to an array of matching agents or null if error
   */
  public async resolveAgentsByCapabilities(
    capabilities: string[],
    matchAll: boolean = false
  ): Promise<Array<{ name: string; data: any }> | null> {
    try {
      // Validate inputs
      if (!capabilities || !Array.isArray(capabilities)) {
        console.error("Invalid capabilities: must be an array");
        return null;
      }
      ...
      if (validCapabilities.length === 0) {
        console.error("No valid capabilities provided");
        return null;
      }

      // Get agents from database
      const agents = await this.registry.getAgentsByCapabilities(
        validCapabilities,
        matchAll
      );
      ...
      if (!agents || agents.length === 0) {
        console.log(
          `No agents found with capabilities: ${validCapabilities.join(", ")}`
        );
        return [];
      }

      // Parse and return agent data
      const resolvedAgents: Array<{ name: string; data: any }> = [];

      for (const agent of agents) {
        try {
          // Extract JSON from formatted card
          let jsonString = agent.card;
          const jsonStartIndex = agent.card.indexOf("{");
          if (jsonStartIndex > 0) {
            jsonString = agent.card.substring(jsonStartIndex);
          }
          ...
          console.warn(
            `Failed to parse agent data for ${agent.name}:`,
            this.sanitizeErrorMessage(parseError)
          );
      ...
      console.log(
        ...
        } agents with capabilities: ${validCapabilities.join(", ")}`
      );
      return resolvedAgents;
    } catch (error) {
      // Log error but return null instead of throwing
      console.error(
        "Error resolving agents by capabilities:",
        this.sanitizeErrorMessage(error)
      );
      return null;
    }
  ...
  /**
   * Get capability suggestions based on existing agents
   * @returns Promise that resolves to an array of unique capabilities
   */
  public async getAvailableCapabilities(): Promise<string[]> {
    try {
      // Get all agents from database
      const sql = "SELECT card FROM agents";

      return new Promise<string[]>((resolve, reject) => {
        this.registry["db"].all(
          sql,
          [],
          (err: Error, rows: Array<{ card: string }>) => {
            if (err) {
            ...
            for (const row of rows) {
              try {
                // Decrypt if needed
                let decryptedCard = row.card;
                if (
                  this.registry["options"].encryptData &&
                  this.registry["encryptionKey"]
                ) {
                  decryptedCard = this.registry["decryptData"](row.card);
                }
                ...
                // Look for capabilities in metadata
                let capabilities: string[] = [];
                if (agentData.metadata && agentData.metadata.capabilities) {
                  capabilities = agentData.metadata.capabilities;
                } else if (agentData.capabilities) {
                ...
                console.warn(
                  `Skipping invalid agent card: ${this.sanitizeErrorMessage(
                    parseError
                  )}`
                );
              }
            ...
            resolve(Array.from(allCapabilities).sort());
          }
        );
      });
    } catch (error) {
      console.error(
        "Error getting available capabilities:",
        this.sanitizeErrorMessage(error)
      );
      return [];
    }
